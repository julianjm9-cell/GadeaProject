"""
Cambridge Trainer - lanzador de escritorio.
...
HTML_FILE = "apps/cambridge/index.html"
DATA_DIR_NAME = "CambridgeTrainer_Data"
DEFAULT_API_KEY = ""
DEFAULT_LICENSE_SERVER_URL = "http://127.0.0.1:8787"
DEFAULT_PROFILE = (
    "The student is preparing for Cambridge English exams (B2 First, C1 Advanced "
    "or C2 Proficiency), especially Speaking, Writing, Reading and Use of English. "
    "The output should be natural, accurate, exam-ready and easy to adapt. "
    "Prioritise Cambridge criteria, clear structure, task fulfilment, register, "
    "cohesion, pronunciation notes, grammar range, collocations, useful expressions "
    "and common mistakes for Spanish-speaking learners."
)
...
GROQ_BASE_HEADERS = {
    ...
    "User-Agent": "CambridgeTrainer/1.0 (+https://localhost)",
}
DEFAULT_STATE: dict[str, Any] = {
    ...
    "writingPractice": {"taskType": "essay", "level": "C1 Advanced", "prompt": "", "answer": "", "feedback": ""},
...
def ensure_structure() -> dict[str, Path]:
    ...
    if not paths["readme"].exists():
        paths["readme"].write_text(
            "Cambridge Trainer - carpeta de datos\n"
            "================================\n\n"
            "state.json: historial, temas completados y configuraciÃ³n.\n"
            "license.txt: clave de licencia CAMBRIDGE TRAINER del usuario.\n"
...
def backend_server_json(path: str, payload: dict[str, Any] | None = None, timeout: int = 120) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            "No se pudo conectar con el servidor CAMBRIDGE TRAINER. "
...
def license_payload(extra: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not license_key:
        raise RuntimeError("Falta la clave de licencia CAMBRIDGE TRAINER.")
    return {"license_key": license_key, "client_app": "cambridge", **(extra or {})}
...
def multipart_body(fields: dict[str, str], file_field: str, filename: str, content_type: str, data: bytes) -> tuple[bytes, str]:
    boundary = f"----CambridgeTrainer{int(time.time() * 1000)}"
    chunks: list[bytes] = []
...
class GadeaHandler(SimpleHTTPRequestHandler):
    server_version = "CambridgeTrainerLocal/1.0"
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if route == "/api/ocr":
                return self.send_json(ocr_image(payload))
            ...
            if route == "/api/export-db":
                filename = safe_filename(
                    ...
                    f"CambridgeTrainer_BD_{datetime.now().date().isoformat()}.json",
                )
                target = self.paths["exports"] / filename
...
def show_error(message: str) -> None:
    try:
        ...
        mb.showerror("Cambridge Trainer", message)
    except Exception:
        print(message)
